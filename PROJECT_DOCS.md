...
# Май 2025: Ежедневный автопостинг по расписанию (Telegram Invite)

## Основные изменения
- Автопостинг теперь реализован как бесконечный цикл: каждый день в указанные часы (по Москве) генерируется и публикуется следующий пост по выжимке.
- Используется московское время (`Europe/Moscow`) для всех расчётов расписания, независимо от часового пояса сервера.
...
## Как это работает
- При нажатии "Запустить автопостинг":
  - Если уже есть активная задача, она получает сигнал остановки и завершает работу.
  - Запускается новая задача с новым расписанием.
  - В цикле автопостинга периодически проверяется флаг остановки.
- Все публикации происходят строго по московскому времени.
- Статус автопостинга и последнее расписание отображаются на фронте и сохраняются между перезагрузками страницы.

## Преимущества
- Можно менять расписание автопостинга без остановки сервиса.
- Исключено дублирование постов и конфликт задач.
- Удобная диагностика через логи.
