...
# Май 2025: Переход на асинхронную публикацию постов в Telegram (python-telegram-bot 22.x+)

## Причина изменений
- В проекте используется python-telegram-bot версии 22.x+, которая полностью асинхронная.
- Старый синхронный код публикации постов (через Bot.send_message) приводил к ошибкам вида 'coro' object has no attribute ...' при запуске в Celery-задачах.
- Требовалось обеспечить корректную работу публикации постов и автопостинга с поддержкой новых версий библиотеки.

## Основные изменения

### 1. publish_post_task (tasks.py)
- Функция переписана на асинхронный стиль: теперь отправка сообщения происходит внутри async-функции send(), которая вызывается через asyncio.run().
- Используется только await bot.send_message(...), что совместимо с python-telegram-bot 22.x+.
- Добавлено подробное логирование ошибок публикации (chat_id, токен, длина текста, тип ошибки).
- Добавлена обработка типовых ошибок: чат не найден, бот заблокирован, недостаточно прав.
- Логика force-публикации (повторная публикация) теперь корректно работает с учётом статуса поста.

### 2. autopost_task (tasks.py)
- Функция автопостинга также переписана на асинхронный стиль: все отправки сообщений происходят через await bot.send_message(...) внутри async-функции send_all().
- Вызов осуществляется через asyncio.run(send_all()), что позволяет использовать современные версии библиотеки.
- Добавлена задержка между постами через await asyncio.sleep(5).

### 3. Диагностика и рекомендации
- В процессе отладки выявлено, что python-telegram-bot 20+ требует асинхронного вызова всех методов.
- Для старых проектов возможен откат до версии 13.15, если требуется синхронная работа.
- Для новых проектов рекомендуется использовать только async-стиль и актуальные версии библиотеки.
- Вся логика, связанная с Telethon, OpenAI, анализом и генерацией, не требует изменений и работает как прежде.
