...
- **Функционал**:
  - Парсинг подписчиков из супергрупп Telegram
  - Сбор уникальных username'ов
  - Фильтрация ботов
  - Экспорт результатов в файл
- **API эндпоинты**:
  - `GET /parser` - страница парсера
  - `POST /api/parse` - запуск парсинга
  - `GET /api/parse/status` - статус парсинга
  - `GET /api/parse/download/<filename>` - скачивание результатов
- **Конфигурация**:
  - Использует существующие Telegram-аккаунты из `config.json`
  - Настраиваемое количество ID для сбора
