...
def stats_history():
    ...
    if period=='week':
        ...
        cursor.execute("SELECT DATE_FORMAT(created_at,'%Y-%m-%d') AS ts, COUNT(*) AS count "
                       "FROM invite_logs WHERE created_at>=%s GROUP BY ts ORDER BY ts", (start,))
    else:
        start = now - timedelta(hours=24)
        cursor.execute("SELECT DATE_FORMAT(created_at,'%Y-%m-%dT%H:00:00Z') AS ts, COUNT(*) AS count "
                       "FROM invite_logs WHERE created_at>=%s GROUP BY ts ORDER BY ts", (start,))
...
def api_logs_csv():
    ...
    cnx = mysql.connector.connect(host=DB_HOST, port=DB_PORT, user=DB_USER, password=DB_PASSWORD, database=DB_NAME)
    cursor = cnx.cursor()
    cursor.execute("SELECT id,task_id,account_name,channel_username,phone,status,reason,created_at FROM invite_logs")
    rows = cursor.fetchall()
    cursor.close(); cnx.close()
    sio = StringIO(); w=csv.writer(sio)
    w.writerow(['id','task_id','account_name','channel_username','phone','status','reason','created_at'])
    w.writerows(rows)
    return sio.getvalue(),200,{'Content-Type':'text/csv','Content-Disposition':'attachment; filename=invite_logs.csv'}
